...
class Invoice(Base, TimestampMixin):
    ...
    Lưu trữ các hóa đơn được tạo cho users
    """
    ...
    # Status
    status = Column(String(50), default="draft", index=True)  # draft, pending, paid, overdue, cancelled
...
class Payment(Base, TimestampMixin):
    ...
    Lưu trữ các thanh toán liên quan đến invoices hoặc transactions
    """
    ...
    amount = Column(DECIMAL(20, 8), nullable=False)
    currency = Column(String(20), nullable=False, default="USD")
    payment_method = Column(String(50), nullable=False, index=True)  # bank_transfer, credit_card, debit_card, e_wallet, crypto
    
    # Status
    status = Column(String(50), default="pending", index=True)  # pending, processing, completed, failed, refunded
