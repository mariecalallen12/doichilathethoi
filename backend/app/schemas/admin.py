...
# ========== CUSTOMER WALLET BALANCES SCHEMAS ==========

class CustomerWalletBalance(BaseModel):
    """Schema for customer wallet balance"""
    userId: str = Field(..., description="User ID")
    email: str = Field(..., description="User email")
    displayName: Optional[str] = Field(None, description="Display name")
    balances: Dict[str, float] = Field(default_factory=dict, description="Wallet balances by currency")
    totalBalanceUSD: float = Field(default=0.0, description="Total balance in USD")
    createdAt: datetime = Field(..., description="User registration time")
    lastUpdated: Optional[datetime] = Field(None, description="Last balance update time")


class CustomerWalletBalancesResponse(BaseModel):
    """Schema for customer wallet balances response"""
    success: bool = Field(..., description="Operation success status")
    data: List[CustomerWalletBalance] = Field(..., description="List of customer wallet balances")
    pagination: Dict[str, Any] = Field(..., description="Pagination info")
