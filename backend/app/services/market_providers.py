        ...
        try:
            async with self.api_client as client:
                ...
                if data and "rates" in data:
                    rate = data["rates"].get(quote, 1.0)
                    price = float(rate)
                    
                    # Mock 24h change for forex (would need historical data for real calculation)
            ...
            logger.error(f"Error getting Forex price for {pair}: {e}")
...
class MetalsDataProvider:
    ...
    async def get_price(self, symbol: str) -> Optional[PriceData]:
        """Get price for a precious metal"""
        try:
            async with self.api_client as client:
                ...
                if data and "rates" in data and "USD" in data["rates"]:
                    ...
                    # Mock 24h change
                    if "XAU" in symbol:
                        change_24h = 0.15  # Gold typically stable
                    else:
                        change_24h = -0.05  # Silver typically more volatile
            ...
            logger.error(f"Error getting metals price for {symbol}: {e}")
