#!/bin/bash
# Script dọn dẹp dữ liệu cũ, dư thừa, test

set -e

echo "=========================================="
echo "DỌN DẸP DỮ LIỆU CŨ/DƯ THỪA/TEST"
echo "=========================================="
echo ""
...
read -p "Bạn có chắc chắn muốn thực hiện cleanup? (yes/no): " confirm
if [ "$confirm" != "yes" ]; then
    echo "Đã hủy cleanup."
    exit 1
fi
...
if [ -z "$REDIS_PASSWORD" ]; then
    ...
    docker exec digital_utopia_redis redis-cli --no-auth-warning --scan --pattern "cache:*" | xargs -r docker exec digital_utopia_redis redis-cli --no-auth-warning DEL 2>&1 || true
    # Xóa session cũ (nếu có)
    docker exec digital_utopia_redis redis-cli --no-auth-warning --scan --pattern "session:*" | xargs -r docker exec digital_utopia_redis redis-cli --no-auth-warning DEL 2>&1 || true
else
...
# Xóa file cũ hơn 1 năm và không được reference trong database
docker exec digital_utopia_backend find /app/uploads -type f -mtime +365 -delete 2>&1 || true
echo "✓ Uploads cleanup completed"
echo ""

# 4. Kiểm tra volumes không được sử dụng
echo -e "${GREEN}[4] Kiểm tra volumes không được sử dụng...${NC}"
docker volume ls -q | while read volume; do
    if ! docker ps -a --format '{{.Mounts}}' | grep -q "$volume"; then
        echo -e "${YELLOW}Warning: Volume $volume không được sử dụng bởi container nào${NC}"
    fi
done
echo ""

# 5. Reindex và optimize database
echo -e "${GREEN}[5] Tối ưu hóa database...${NC}"
docker exec digital_utopia_postgres psql -U postgres -d digital_utopia -c "REINDEX DATABASE digital_utopia;"
docker exec digital_utopia_postgres psql -U postgres -d digital_utopia -c "VACUUM FULL ANALYZE;"
